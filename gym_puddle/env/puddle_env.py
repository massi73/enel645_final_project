...
class PuddleEnv(gymnasium.Env):
    def __init__(
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(3,156,156), dtype=np.uint8)
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        grey_img = np.transpose(cv2.resize(self.render(), (156,156)),axes=(2,0,1))
        return grey_img.astype(np.uint8), reward, done, trunc, {}
